...
volatile static int32_t g_rpm = 0;
...
static SemaphoreHandle_t 		g_rpm_set_sem;
...
void motor_control_init()
{
	...
	g_rpm_set_sem = xSemaphoreCreateBinary();
	xSemaphoreGive(g_rpm_set_sem);
...
{
	xSemaphoreTake(g_rpm_set_sem, portMAX_DELAY);
	if(rpm < 500 || rpm > 4000)
	{
		motor_control_stop();
	}
...
void ADC1_IRQHandler(void)
{
	...
	measures_motor_control_int_out();
    /* Enable interrupts  */
    EnableGlobalIRQ(ui32PrimaskReg);
